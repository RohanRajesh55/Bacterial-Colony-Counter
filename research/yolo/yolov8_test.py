...
# Path resolution - derive from script location
...
FULL_REPORT_OUTPUT_DIR = SCRIPT_DIR / "runs" / "full_validation_report"
# Default paths - override with environment variables if needed
SOURCE_PATH = Path(os.environ.get("YOLO_VAL_IMAGES", str(PROJECT_ROOT / "dataset" / "images" / "val")))
# Path to the corresponding label folder
LABELS_PATH = Path(os.environ.get("YOLO_VAL_LABELS", str(PROJECT_ROOT / "dataset" / "labels" / "val")))
...
SINGLE_IMAGE_OUTPUT_DIR = SCRIPT_DIR / "runs" / "single_image_tests"


# --- General Settings ---
WEIGHTS_PATH = Path(os.environ.get("YOLO_WEIGHTS", str(SCRIPT_DIR / "runs" / "colony_yolo" / "yolov8m_optimized_run2" / "weights" / "best.pt")))
